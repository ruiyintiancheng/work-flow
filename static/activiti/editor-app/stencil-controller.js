  ...
  .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function($rootScope, $scope, $http, $modal, $timeout) {
    ...
    $scope.editorFactory.promise.then(function() {
      ...
      /*
             StencilSet items
             */
      $http({ method: 'POST', url: KISBPM.URL.getStencilSet() /* headers: { 'token': token }*/}).success(function(response, status, headers, config) {
        ...
        var morphRoles = []
        var data = JSON.parse(response.data.item.result)

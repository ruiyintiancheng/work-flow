  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
    function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
      ...
      function fetchModel(modelId) {
        var modelUrl = KISBPM.URL.getModel()
        // var token = localStorage.getItem('Admin-Token')
        $http({ method: 'POST', url: modelUrl, data: { 'modelId': modelId }})
          .success(function(data, status, headers, config) {
            const retVal = JSON.parse(JSON.parse(data))

            $rootScope.editor = new ORYX.Editor(retVal)
            $rootScope.modelData = angular.fromJson(retVal)
